...
class NaviIntegration:
    ...
    def lidarCallback(self, lidarscan):
        ...
        if self.dot_dir > 0:    # Forward
            points = [list(cloud_point) for cloud_point in cloud_points if cloud_point[0] <= 10.0 and cloud_point[2] < self.MAX_HEIGHT and cloud_point[2] > self.MIN_HEIGHT] 
        else:                   # Backward
            points = [list(cloud_point) for cloud_point in cloud_points if cloud_point[0] >= -10.0 and cloud_point[2] < self.MAX_HEIGHT and cloud_point[2] > self.MIN_HEIGHT] 
    ...
    def direction_function(self):                
        ...
        dir_robot_goal = np.array([self.goal.target_pose.pose.position.x - self.pose_x, self.goal.target_pose.pose.position.y - self.pose_y])
        ...
        self.dist_amcl_goal = math.sqrt(math.pow((self.goal.target_pose.pose.position.x - self.pose_x), 2) + math.pow((self.goal.target_pose.pose.position.y - self.pose_y), 2))
    ...
    def move(self):   
        ...
        if (self.dot_dir_list[-1] * self.dot_dir_list[-2] < 0) and (self.dot_dir_list[-2] * self.dot_dir_list[-3] < 0):
                ...
                if self.dot_dir < 0:                 # When the robot needs a rotation in place to reach the goal position 
                    if self.dist_amcl_goal > self.NEAR_AREA_DIST:     # When the robot is near the goal point: "Fine" controller   
                        ...
                        while((rospy.Time.now() < endTime) & self.rotation_trigger1 == True):
                            ...
                            print("Rotating in place (backward), angular velocity: ", vel_msg.angular.z, " ", rospy.Time.now())
                        ...
                        while((rospy.Time.now() < endTime) & self.rotation_trigger2 == True):
                            ...
                            cmd_vel_publisher.publish(vel_msg)
                            print("Slightly rotating in place (backward), angular velocity: ", vel_msg.angular.z, " ", rospy.Time.now())
                    ...
                    if self.dist_amcl_goal > self.NEAR_AREA_DIST:     # When the robot is near the goal point: "Fine" controller   
                        ...
                        while((rospy.Time.now() < endTime) & self.rotation_trigger2 == True):
                            ...
                            cmd_vel_publisher.publish(vel_msg)
                            print("Slightly rotating in place (forward), angular velocity: ", vel_msg.angular.z, " ", rospy.Time.now())
            ...
            if (self.dist_amcl_goal < self.NEAR_AREA_DIST) & self.trigger[self.goal_cnt]:     # When the robot is far from the goal point: "row tracer" controller                                    
