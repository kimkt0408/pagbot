...
private:
    ...
    Symbol lastKey;
    ros::Subscriber odomSub, imuSub, gpsSub, vCloudSub;
    ros::Publisher posePub, trajectoryPub, mapCloudPub, gpsMapPub; 
    ...
    float gps_noise_threshold = 4e-4; //5e-2, 2023-09-14: 4e-4;
...
public:
    GtsamOptimizer(ros::NodeHandle& nh) {
        ...
        priorPoseNoise  = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1e-4, 1e-4, 1e-4, 1e-2, 1e-2, 1e-2).finished()); // rad,rad,rad,m, m, m
        odometryNoise  = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1, 1, 1, 1, 1, 1).finished()); // rad,rad,rad,m, m, m
...
// Conversion of translation vector and quaternion to a 4x4 matrix
    ...
    //         odom[1] = pitch;
    //         odom[2] = yaw;
    //         odom[3] = msg->pose.position.x;
    //         odom[4] = msg->pose.position.y;
    //         odom[5] = msg->pose.position.z;
    ...
    //         delta_x = msg->pose.position.x - prev_odom[3];
    //         delta_y = msg->pose.position.y - prev_odom[4];
    //         delta_z = msg->pose.position.z - prev_odom[5];
    ...
    //         prev_optimized_odom[1] = curr_pitch;
    //         prev_optimized_odom[2] = curr_yaw;
    //         prev_optimized_odom[3] = prev_optimized_odom[3] + delta_x;
    //         prev_optimized_odom[4] = prev_optimized_odom[4] + delta_y;
    //         prev_optimized_odom[5] = prev_optimized_odom[5] + delta_z;
    ...
    //     prev_odom[1] = pitch;
    //     prev_odom[2] = yaw;
    //     prev_odom[3] = msg->pose.position.x;
    //     prev_odom[4] = msg->pose.position.y;
    //     prev_odom[5] = msg->pose.position.z;
    ...
    void addGPSFactor() {    
        ...
        while (!gpsQueue.empty())
        {
            ...
            else
            {
                ...
                gpsMapFrameVisualization(thisGPS);
    ...
    void graphUpdate() {
        ...
        if (vec_odom.size() == 1){
            ...
            // noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Variances((Vector(6) << ).finished()); // rad*rad, meter*meter
            ...
            // cout << vec_odom.size()-1 << " " << pose[3] << " " << pose[4] << " " << pose[5] << endl;
        }
    }
